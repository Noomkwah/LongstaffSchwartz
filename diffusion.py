...
class BrownianIncrements:
    ...
    def __call__(self, t0: float, t1: float, d: int, N: int, corr_matrix: np.ndarray = None) -> np.ndarray:
        ...
        """
        delta_t = t1 - t0
        increments = np.random.normal(loc=0, scale=np.sqrt(delta_t), size=(N, d) if d>1 else (N,))
...
class MonteCarloPaths:
    """
    Generic Class to store Monte Carlo paths.
    
    ---------------
    Attributes:
        times, paths, N_samples, maturity
    """
    
    def __init__(self, paths: np.ndarray, times: np.ndarray) -> None:
        """
        Instantiates a Monte Carlo Path.
        
        ---------------
        Arguments:
            times: np.array, the times for which the samples are known.
            paths: np.array, of size (times, N_samples). The paths themselves.        
        """
        ...
        self.maturity = times[-1]
    
    def __call__(self, t: float) -> np.ndarray:
        if t > self.maturity:
            raise IndexError(f"Paths only stored until T = {self.maturity}. Unable to provide path for t = {t}.")
        elif t == 0:
            return self.paths[0]
        else:
